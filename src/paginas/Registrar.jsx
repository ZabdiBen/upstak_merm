...
const Registrar = () => {
  ...
  const handleSubmit = async e => {
    ...
    try {
      const {data} = await axios.post(`${import.meta.env.VITE_BACKEND_URL}`, {nombre, email, password })
  ...
  return (
    <>
      ...
      <form className="my-10 bg-white shadow rounded-lg px-10 py-5" onSubmit={handleSubmit}>
        <div>
          <label className="uppercase text-gray-600 block" htmlFor="nombre">
            Nombre
          </label>
          <input
            id="nombre"
        ...
        <div>
          <label className="uppercase text-gray-600 block" htmlFor="password2">
            Repetir Password
          </label>
          <input
            id="password2"
